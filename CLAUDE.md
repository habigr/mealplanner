...
STATE: **v177 is LIVE** (pushed 2026-07-02) — bottom tab bar restyled to an authentic iOS look (mobile, render-only, FIP-safe): no more navy "All Trips" chip (now a clean tab w/ 2×2 grid icon), SF Pro labels, cohesive accent, glass + 0.5px hairline, iOS press-dim. node --check pass; visual check on a phone recommended.
