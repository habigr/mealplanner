...
0. ~~**Fabricated recipes from failed URL imports**~~ — FIXED v78. When Microlink couldn't fetch a page (no title), the app still called Claude with empty content and the prompt told it to use training-data knowledge → it invented a recipe the user thought was real. v78: if no page title is fetched, error + STOP before any Claude call. NOTE/FOLLOW-UP: even on success, Claude reconstructs the recipe from training data + the page title, NOT the literal page HTML — so it can still be inaccurate. The v77 confirm-the-page step mitigates; a fuller fix would fetch and parse actual page content. Worth revisiting.
