...
STATE: **v125 shipped (2026-06-18, pushed live)** — added read-only `window.diagStorage()` to measure exactly what fills localStorage (per key / per trip / embedded-image counts); 25/25 guest (Robbie). **⏳ WAITING ON ROBBIE: run `window.diagStorage()` on the FULL PHONE on the live site (habigr.github.io — NOT localhost, which is a separate near-empty origin) and paste the output — that decides v126's cut.** v124 (joined trips show on a storage-full device) shipped + live, 25/25, also still awaiting Robbie's phone confirmation that the joined trip appears. **PRIORITY THREAD (Robbie, 2026-06-18): storage → archive → admin.** NEXT = **v126: the actual cache cut, per BACKLOG #33** — backup-cap to ~3 most-recent trips (SAFE — current trip always kept; backup is a tertiary copy) + whatever `diagStorage()` flags as heaviest (likely embedded photos → hosted-URL / IndexedDB); keep offline working. **DO NOT strip images from the backup — `restoreFromBackup`→`commitAndSync` would push blanks to Firebase (data loss).** → then **#22 archive** (per-user, design LOCKED) → **#18/19/20 admin & access** (global-admin model, three delete levels, security rules last + staged live). Also queued: **#32 copy/duplicate trip**. (Cooking-mode C0 still queued but no longer the immediate next.) v123 shipped + confirmed earlier. FIP-safe: no write-path change, nothing deleted. **v122** (adjusted-amount override, dish-level single source of truth) also shipped + confirmed 25/25. Small related follow-up if Robbie wants it: the grocery "Edit ingredient" popup now edits the override (not the base) — flip it back to base editing if that's preferred. Storage note: iOS localStorage (~5MB) fills with trips/images/backups; if it bites again, options are real (non-deleting) reductions — e.g. stop keeping a full local backup copy per trip (saveLocalBackup), or store recipe images by URL only.
