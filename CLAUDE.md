...
STATE: **v124 shipped (2026-06-18, pushed live)** — joined trips now show on a storage-full device (home list driven by the Firebase account trip-list, not local cache); 25/25 verified by Robbie, awaiting his phone confirmation that the joined trip appears once GitHub Pages deploys. **NEW PRIORITY THREAD (Robbie, 2026-06-18): storage → archive → admin.** NEXT = **v125: slim the local cache (BACKLOG #33)** so the device stops filling up (images by URL, drop redundant local backups, prune stale cached trips; keep offline working) → then **#22 archive** (per-user, design LOCKED) → **#18/19/20 admin & access** (global-admin model, three delete levels, security rules last + staged live). Also queued from this session: **#32 copy/duplicate trip**. (Cooking-mode C0 remains queued but is no longer the immediate next — superseded by the storage/archive/admin thread.) v123 (iOS storage-full no longer blocks the cloud save) shipped + confirmed earlier. FIP-safe: no write-path change, nothing deleted. **v122** (adjusted-amount override, dish-level single source of truth) also shipped + confirmed 25/25. Small related follow-up if Robbie wants it: the grocery "Edit ingredient" popup now edits the override (not the base) — flip it back to base editing if that's preferred. Storage note: iOS localStorage (~5MB) fills with trips/images/backups; if it bites again, options are real (non-deleting) reductions — e.g. stop keeping a full local backup copy per trip (saveLocalBackup), or store recipe images by URL only.
