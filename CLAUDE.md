...
STATE: **v138 is BUILT, not yet pushed/verified** (color-scheme header — replaces the entire photo-banner system, NO photos). Working tree has uncommitted index.html + doc changes. ⚠️ The `banner-wip` branch (photo shared-gallery #28) is **OBSOLETE — do NOT ship it** (superseded; delete the branch eventually). Backup tag **`pre-v138-photo-removal`** = one-command rollback.

**v138 — DONE in code, PENDING verification before push:**
- Removed ALL photo machinery; added 8 Color Schemes (Midnight/Ocean/Forest/Sunset/Plum/Slate/Cherry/Pride) in Trip settings, saved on `currentEvent.theme`, synced to all trip users. Scheme tints the whole app accent + header gradient (`--hg`); header shows name + dates. `mapTheme()` back-compat. node --check pass; no dup defs; −57 net lines.
- **⚠️ BEFORE PUSH (Robbie):** (1) run `window.runRegressionTests()` → 25/25 (logged in, trip open). (2) Visual check: open a trip, change the Color Scheme dropdown → header gradient + buttons/accents retint live; trip name+dates show; readable in light AND dark mode; check Pride. (3) **2-device sync test on a THROWAWAY trip** (NOT FIP): change scheme on device A → appears on device B. (4) Open an OLD trip (FIP has `theme:'fire-island'`/photo) → header shows the mapped scheme (ocean) with no errors, FIP data untouched. Then push (auto-deploys).
