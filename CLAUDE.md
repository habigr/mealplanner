...
29. **All API keys should stick with the TRIP, not the device/user (Robbie).** TODAY: the Anthropic key already syncs with the trip (saveApiKey -> currentEvent.sharedApiKey + commitAndSync; loadSharedApiKey reads it). BUT the Spoonacular image key (getImageApiKey/saveImageApiKey, IMG_API_KEY_STORAGE) is DEVICE-LOCAL only (made local in v92) — so other devices/users can't source images. QUICK FIX (small, mirrors the Anthropic pattern): on saveImageApiKey also write currentEvent.sharedImageApiKey + commitAndSync; on trip load (loadSharedApiKey) pull sharedImageApiKey into local if missing. Robbie plans the CLEANER version (admin-managed keys) as part of admin mode (#18-20) — fold the proper "all keys live on the trip / admin-set" model there. Decide now vs later: quick sync fix is low-risk; clean version waits for #18.
...
25. **Image sourcing misses simple AI/no-URL recipes (Robbie) — list only.** AI-generated "Peach, Arugula & Burrata Salad" pulled no picture. sourceRecipeImage tries the recipe URL first (none here) then Spoonacular complexSearch by the FULL name — a long/specific title (commas, &) rarely matches Spoonacular's ~5k titles, so it returns nothing. DECISION (Robbie): URL/Microlink image stays ALWAYS the first solution. (a) DONE v108 — simplify the Spoonacular query. (b) NOW NEEDED — confirmed still-blank case: "Peach, Arugula & Burrata Salad" (niche, no Spoonacular match) stays blank in BOTH detail view + card thumbnail, even with the key. Two-part fix to make "every recipe has an image" true: (i) PROACTIVE sourcing — fire sourceRecipeImage on recipe SAVE/EDIT (saveDishFromModal bankOnly) too, not just detail-open/backfill, + a self-heal pass for library recipes missing images (mirror the grocery-name self-heal); (ii) GENERAL food-photo FALLBACK (Pexels — Robbie leaning — or Unsplash, free key) when Spoonacular returns nothing, so niche dishes still get a fitting photo. Pick service + key, then build. Note: _imageFetchAttempted marks a recipe "tried" for the session, so after improving, a page reload is needed to retry the ones that already failed.
...
STATE (session ended at v111): shipped v80→v111. 25/25 regression confirmed at v101 AND v109. Working tree clean. Robbie wants the **COOKING-MODE UPDATE (C0)** next. Big things done since v101: A2 (AI input clears), #23 cook time end-to-end, #24 paste pulls steps, #25 smarter image search + Pexels fallback (verified) + image keys sync with trip (#29 mostly done), #26 edit dish item from grocery pill, #27B robust AI categorization (salt/pepper fixed), deleted-trips fix (v106), online-photos fix (v107), grocery section categorizer fix (v101).

**START HERE NEXT: COOKING-MODE REDESIGN (C0)** — see the COOKING MODE section above for the full spec (C0a–C0h). Priority order: C0a timers persist + parallel (also a bug: timer dies on exit), C0f immediate Back/Exit, C0g fix clock/status-bar overlap, C0h remove the "cooking for # people" adjuster, C0d show DISH PICTURE not emoji + clean view, C0b multi-dish dashboard, C0c coordination timeline, C0e QoL. Fold in C1/C2. This is its own focused session — read the C0 spec first.

1. **COOKING-MODE REDESIGN (C0) — what Robbie wants next.** See the COOKING MODE section (C0a–C0h). Lead with C0a (persistent/parallel timers — also fixes the timer-dies-on-exit bug), then the quick wins C0f (immediate Back), C0g (clock overlap), C0h (remove people adjuster), C0d (dish picture not emoji). Its own focused session.
2. **Smaller queued (safe):** proactive image sourcing on save (fire sourceRecipeImage in saveDishFromModal bankOnly + a self-heal, so recipes auto-fill without Backfill); **#27A** grocery section headers consistent on all screens (un-hide .grocery-section-row in the ≤768px media query, line ~603, + restyle for the card layout).
3. **Existing-duplicate cleanup (#3) — tool is LIVE:** run `cleanupDuplicateRecipes()` (dry-run) then `cleanupDuplicateRecipes({apply:true})`.
4. **#28 trip header photos sync** (per-device localStorage gallery; sync change, test on 2 devices) · **App Admin/permissions cluster #18→#19→#20** (its own session). #18 first (split universal vs trip settings, collapsible), then #19 (user directory + cook dropdown), then #20 (Firebase security rules — untested rules can lock out all users; stage carefully WITH Robbie).
